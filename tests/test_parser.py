...
@pytest.mark.parametrize("filepath", TEST_CSVS)
def test_csv_payload_generator(filepath):
    index = 0
    for index, result in enumerate(parser.csv_payload_generator(filepath), start=1):
        assert isinstance(result, dict)
    assert index > 0


@pytest.mark.parametrize("filepath", TEST_CSVS)
def test_csv_payload_generator_with_mutator(filepath):
    ...
    for result in parser.csv_payload_generator(filepath, mutator=replace_with_spam):
